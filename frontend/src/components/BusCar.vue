<template>
  <div class="container mt-4">
    ...
    <div v-if="articulos.length" class="mt-4">
      ...
      <ul>
        <li v-for="a in articulos" :key="a._id">
          <span v-html="resaltar(a.marca + ' ' + a.modelo, termino)"></span>
          -
          <small
            class="text-muted"
            v-html="resaltar(a.descripcion, termino)"
          ></small>
        </li>
      </ul>
    </div>
...
<script setup>
// IMPORTANTE HACEMOS USO DE USEROUTE PARA ACCEDER A LOS PARÁMETROS DE LA RUTA Y LAS BÚSQUEDAS DESDE EL FRONTEND
